...
if (isset($_POST['update_status'])) {
    ...
    $catatan_admin  = mysqli_real_escape_string($conn, trim($_POST['catatan_admin'] ?? ''));
    $allowed_status = ['pending','diterima','ditolak'];
    if (!in_array($status, $allowed_status)) {
        $err = "Status tidak valid.";
    } else {
        $tanggal_proses = ($status !== 'pending') ? "tanggal_proses=CURDATE()," : "";
        mysqli_query($conn, "
            UPDATE pengajuan
            SET status='$status', $tanggal_proses
                catatan_admin='$catatan_admin',
                id_admin='{$_SESSION['user_id']}'
            WHERE id_pengajuan='$id'
        ");
        $msg = "Status pengajuan berhasil diperbarui menjadi <b>" . ucfirst($status) . "</b>.";
    }
}
...
if ($filter_bansos) $where .= " AND p.id_bansos='$filter_bansos'";
if ($filter_tgl)    $where .= " AND DATE(p.tanggal_pengajuan)='$filter_tgl'";
if ($search)        $where .= " AND (m.nama LIKE '%$search%' OR m.nik LIKE '%$search%')";
...
$stat_diterima = mysqli_fetch_assoc(mysqli_query($conn,"SELECT COUNT(*) c FROM pengajuan WHERE status='diterima'"))['c'];
$stat_ditolak  = mysqli_fetch_assoc(mysqli_query($conn,"SELECT COUNT(*) c FROM pengajuan WHERE status='ditolak'"))['c'];
...
$total_filtered = mysqli_fetch_assoc(mysqli_query($conn,"
    SELECT COUNT(*) c FROM pengajuan p
...
$data = mysqli_query($conn,"
    SELECT p.*, m.nama AS nama_pemohon, m.nik,
           jb.nama_jenis,
           u.nama AS nama_admin
    FROM pengajuan p
    LEFT JOIN masyarakat m ON p.nik=m.nik
    LEFT JOIN jenis_bantuan jb ON p.id_bansos=jb.id
...
$jenis_list = mysqli_query($conn,"SELECT * FROM jenis_bantuan ORDER BY nama_jenis ASC");

$qs = http_build_query(array_filter([
    'q'      => $search,
    'status' => $filter_status,
    'bansos' => $filter_bansos ?: '',
    'tgl'    => $filter_tgl,
]));
...
<head>
<meta charset="UTF-8">
<title>Super Admin — Monitor Pengajuan</title>
...
.modal-box { background:var(--sa-surface);border:1px solid var(--sa-border);border-radius:16px;padding:32px;width:100%;max-width:480px;animation:pop-in .25s ease; }
...
.progress-bar { background:var(--sa-border);border-radius:4px;height:6px;overflow:hidden;margin-top:8px; }
.progress-fill { height:100%;border-radius:4px;background:linear-gradient(90deg,var(--sa-gold),var(--sa-gold2)); }
</style>
</head>
<body>
...
<main class="sa-main">
  ...
  <div class="sa-topbar">
    <div>
      <h1 class="sa-page-title">Monitor Pengajuan</h1>
      <p class="sa-page-sub">Pantau & kelola seluruh pengajuan bantuan sosial</p>
    </div>
    <div class="sa-breadcrumb">
      <a href="index.php">Dashboard</a> / Pengajuan
    </div>
  </div>
  ...
  <div class="sa-stats" style="grid-template-columns:repeat(4,1fr);margin-bottom:28px">
    <div class="sa-stat-card gold">
      ...
      <div class="sa-stat-label">Total Pengajuan</div>
    </div>
    <div class="sa-stat-card" style="border-top:3px solid #fbbf24">
      <div class="sa-stat-icon">⏳</div>
      <div class="sa-stat-num"><?= $stat_pending ?></div>
      <div class="sa-stat-label">Menunggu</div>
      <?php if($stat_total > 0): ?>
      <div class="progress-bar"><div class="progress-fill" style="width:<?= round($stat_pending/$stat_total*100) ?>%;background:#fbbf24"></div></div>
      <?php endif; ?>
    </div>
    <div class="sa-stat-card green">
      ...
      <?php if($stat_total > 0): ?>
      <div class="progress-bar"><div class="progress-fill" style="width:<?= round($stat_diterima/$stat_total*100) ?>%;background:var(--sa-green)"></div></div>
      <?php endif; ?>
    </div>
    <div class="sa-stat-card red">
      ...
      <?php if($stat_total > 0): ?>
      <div class="progress-bar"><div class="progress-fill" style="width:<?= round($stat_ditolak/$stat_total*100) ?>%;background:var(--sa-red)"></div></div>
      <?php endif; ?>
    </div>
  </div>
  ...
  <div class="sa-card">
    ...
    <form method="GET" class="sa-filter-bar" style="flex-wrap:wrap">
      <input type="text" name="q" class="sa-search" placeholder="🔍 Cari nama / NIK pemohon..." value="<?= htmlspecialchars($search) ?>" style="min-width:200px">
      ...
      <select name="bansos" class="sa-select-filter" onchange="this.form.submit()">
        ...
        <?php
        $jenis_list = mysqli_query($conn,"SELECT * FROM jenis_bantuan ORDER BY nama_jenis ASC");
        while ($j = mysqli_fetch_assoc($jenis_list)):
        ?>
      ...
      <a href="monitor_pengajuan.php" class="sa-btn sa-btn-ghost">Reset</a>
    </form>

    <p style="font-size:12px;color:var(--sa-muted);margin-bottom:16px">
      Menampilkan <b style="color:var(--sa-text)"><?= $total_filtered ?></b> pengajuan
      <?php if($total_pages>1): ?> · Halaman <b style="color:var(--sa-text)"><?= $page ?></b>/<?= $total_pages ?><?php endif; ?>
    </p>
      ...
      <table class="sa-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <div style="font-weight:600"><?= htmlspecialchars($d['nama_pemohon'] ?? '—') ?></div>
              <div style="font-size:11px;color:var(--sa-muted);font-family:monospace"><?= htmlspecialchars($d['nik']) ?></div>
            </td>
            <td>
              <span class="badge badge-admin"><?= htmlspecialchars($d['nama_jenis'] ?? '—') ?></span>
            </td>
            <td style="max-width:200px">
              <div style="font-size:12px;color:var(--sa-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:180px">
                <?= htmlspecialchars($d['alasan'] ?? '—') ?>
              </div>
            </td>
            <td style="font-size:12px;color:var(--sa-muted)"><?= $d['tanggal_pengajuan'] ? date('d/m/Y', strtotime($d['tanggal_pengajuan'])) : '—' ?></td>
            <td><?= status_badge($d['status']) ?></td>
            <td>
              <button type="button" class="sa-btn sa-btn-blue" style="padding:5px 12px;font-size:12px"
                onclick="openModal(<?= $d['id_pengajuan'] ?>,'<?= addslashes(htmlspecialchars($d['nama_pemohon']??'')) ?>','<?= $d['status'] ?>','<?= addslashes(htmlspecialchars($d['catatan_admin']??'')) ?>')">
                ⚙️ Kelola
              </button>
            </td>
          </tr>
          <?php endwhile; ?>
          <?php if (!$found): ?>
          <tr><td colspan="7"><div class="sa-empty"><div class="sa-empty-icon">📭</div><p>Tidak ada pengajuan ditemukan.</p></div></td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
...
<div class="modal-overlay" id="statusModal">
  <div class="modal-box">
    <div class="modal-title">⚙️ Kelola Pengajuan</div>
    ...
    <form method="POST">
      ...
      <div class="sa-field" style="margin-bottom:16px">
        ...
        <select name="status" id="modalStatus" class="sa-field input" style="width:100%;padding:12px 14px;background:var(--sa-bg);border:1.5px solid var(--sa-border);border-radius:10px;font-size:14px;font-family:Sora,sans-serif;color:var(--sa-text);outline:none;">
...
function openModal(id, nama, status, catatan) {
  ...
  document.getElementById('modalNama').textContent = 'Pemohon: ' + nama + ' · ID #' + id;
